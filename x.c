...
{
    XConvertSelection(
      xw.dpy, XA_PRIMARY, xsel.xtarget, XA_PRIMARY, xw.win, CurrentTime);
}
...
xclippaste(void)
{
    ...
    clipboard = XInternAtom(xw.dpy, "CLIPBOARD", 0);
    XConvertSelection(
      xw.dpy, clipboard, xsel.xtarget, clipboard, xw.win, CurrentTime);
}
...
selrequest(XEvent* e)
{
    ...
    if (xsre->target == xa_targets) {
        ...
        if (xsre->selection == XA_PRIMARY) {
            ...
            fprintf(
              stderr, "Unhandled clipboard selection 0x%lx\n", xsre->selection);
            return;
        }
        if (seltext != NULL) {
            XChangeProperty(xsre->display,
...
xinit(void)
{
    ...
    win.w = term.col * win.cw;
    win.h = term.row * win.ch;
    if (xw.gm & XNegative)
...
xmakeglyphfontspecs(XftGlyphFontSpec* specs,
                    ...
                    int y)
{
    float winx = x * win.cw, winy = y * win.ch, xp, yp;
                    ...
                    int y)
{
    ...
    int winx = x * win.cw, winy = y * win.ch, width = charlen * win.cw;
...
xdrawcursor(void)
{
    ...
    g.mode |=
      ...
      (ATTR_BOLD | ATTR_ITALIC | ATTR_UNDERLINE | ATTR_STRUCK | ATTR_REVERSE);
    ...
    if (win.state & WIN_FOCUSED) {
        switch (win.cursor) {
            ...
            case 4: /* Steady Underline */
                XftDrawRect(xw.draw,
                            &drawcol,
                            curx * win.cw,
                            (term.c.y + 1) * win.ch - cursorthickness,
                            win.cw,
                            cursorthickness);
                break;
            case 5: /* Blinking bar */
        ...
        XftDrawRect(
          xw.draw, &drawcol, curx * win.cw, term.c.y * win.ch, win.cw - 1, 1);
        XftDrawRect(
          xw.draw, &drawcol, curx * win.cw, term.c.y * win.ch, 1, win.ch - 1);
        XftDrawRect(xw.draw,
                    &drawcol,
                    (curx + 1) * win.cw - 1,
                    term.c.y * win.ch,
